...
_STRONG_REJECT_RE = re.compile(r"ไม่สามารถ(ระบุ|ตอบ|ให้)|ไม่มีข้อมูลเกี่ยว|ไม่พบข้อมูล|ไม่เกี่ยวข้องกับ|"
                               # วลีปฏิเสธมาตรฐานของ prompt เลน case — echo คำถามที่มีคำ advice (เช่น
                               # "ไม่ใช่วิธีเปลี่ยนถ่าย...") ไม่ใช่คำแนะนำจริง -> ต้องตัด citation
                               r"ไม่พบเคสที่ตรง|ไม่พบเคสที่เกี่ยว")
...
# เกณฑ์ "แย่งคำถามจากเลน analytic ไปเลน case" — สูงกว่า RERANK_MIN มาก
# คำถามติดเลน analytic (มีคำ aggregate) แต่ถ้า top hit เป็นเอกสาร KM คะแนนสูงมาก = จริงๆถามเนื้อ KM
# ต้อง >0.381 (คะแนนที่ "downtime มากสุด" บังเอิญแตะ KM doc) เพื่อไม่แย่งคำถาม analytic แท้
KM_OVERRIDE_MIN = float(os.getenv("RAG_KM_OVERRIDE_MIN", "0.55"))
...
_AGG_RE = re.compile(r"กี่(เคส|ครั้ง|เรื่อง|อัน|เครื่อง)|ทั้งหมด|อะไรบ้าง|มีเคส|"
...
_MAYBE_AGG_RE = re.compile(
    r"สรุป|สุด|มากกว่า|น้อยกว่า|เคสไหน|อันไหน|ตัวไหน|เครื่องไหน|กี่|จำนวน|เฉลี่ย|รวม|อันดับ|"
...
def _analytic_answer(query, plant, use_model, used):
    ...
    # คำตอบปฏิเสธ (คำถามนอกขอบเขตเคส) ห้าม fallback ไปอ้างเคสทั้งกอง -> citation ว่าง
    # ("ไม่เกี่ยว" ครอบคลุม 'ไม่เกี่ยวกับรายการเคส' ที่ _NOT_FOUND_RE (=ไม่เกี่ยวข้อง) จับไม่ติด)
    reject = _is_reject(text, en) or bool(_STRONG_REJECT_RE.search(text or "")) or ("ไม่เกี่ยว" in (text or ""))
    cited = [] if reject else (named or cases)
    return {"text": text, "citations": [c["case_id"] for c in cited[:20]], "model": used}


def _km_override(query, plant=None):
    """คำถามติดเลน analytic (มีคำ aggregate) แต่จริงๆถามเนื้อหาเอกสาร KM หรือไม่
    จริง = top hit เป็นเอกสาร KM (case_id ขึ้นต้น 'KM_') และ rerank สูงถึง KM_OVERRIDE_MIN
    (คำถาม analytic แท้เช่น 'downtime มากสุด' แตะ KM doc ได้แค่ ~0.38 -> ไม่ถึงเกณฑ์ -> ไม่แย่ง)
    อังกฤษ: reranker ข้าม (คะแนนมั่ว) -> ไม่ override เพื่อไม่เดาผิด"""
    if _is_english(query):
        return False
    hits = search(query, k=10, plant=plant)
    if not hits:
        return False
    rr = _rerank(query, hits)
    if not rr:
        return False
    bi = max(range(len(hits)), key=lambda i: rr[i])
    return rr[bi] >= KM_OVERRIDE_MIN and str(hits[bi].get("case_id", "")).startswith("KM_")
...
def answer(query, k=4, plant=None, model=None):
    ...
    try:
        ...
        # routing: analytic (นับ/สรุป/จัดอันดับ — เห็นเคสทั้งกอง) vs case (retrieval — จับคู่รายเคส/เอกสาร)
        if _AGG_RE.search(query) or \
           (_MAYBE_AGG_RE.search(query) and _classify_lane(query, use_model) == "analytic"):
            # กันเคส KM: คำ aggregate (อะไรบ้าง/เท่าไหร่/สูงสุด/สรุป) อาจมาจากคำถามเนื้อหาเอกสาร KM
            # ไม่ใช่นับเคส MTN — เลน analytic อ่านแต่ metadata เคส ตอบเนื้อ KM ไม่ได้.
            # ถ้า top hit เป็นเอกสาร KM คะแนนสูงมาก (_km_override) = ถามเนื้อ KM จริง -> ไปเลน case
            # (เกณฑ์สูงกัน analytic แท้เช่น "downtime มากสุด" ที่บังเอิญแตะ KM doc คะแนนกลางๆ)
            if _km_override(query, plant):
                cr = _case_answer(query, k, plant, use_model, used)
                if cr and cr.get("citations") \
                   and any(str(c).startswith("KM_") for c in cr["citations"]) \
                   and not _is_reject(cr.get("text", ""), _is_english(query)):
                    return cr
            return _analytic_answer(query, plant, use_model, used)
        return _case_answer(query, k, plant, use_model, used)
    except Exception as e:
        print(f"[RAG] answer ใช้ไม่ได้ ({type(e).__name__}: {e}) -> ตก mock")
        return None


def _case_answer(query, k, plant, use_model, used):
    """เลน case (retrieval): ดึงเคส/เอกสารที่เกี่ยว -> LLM สรุป + citation.
    คืน dict คำตอบ | dict 'ไม่พบเคส' (citations ว่าง) | None (vault ว่าง/พัง -> ให้ app ตก mock)"""

...
STT_ENGINE = os.getenv("STT_ENGINE", "openai").strip().lower()   # "openai" | "gemini" | "whisper"
...
def transcribe_audio(path):
    """ถอดเสียง -> ข้อความ. openai/gemini(ถ้าเลือก) -> server(ถ้าตั้ง) -> local whisper -> mock"""
    if STT_ENGINE == "openai":
        try:
            return _transcribe_openai(path)
        except Exception as e:
            print(f"[STT] openai ใช้ไม่ได้ ({type(e).__name__}: {e}) -> ตกไป whisper")
...
def _transcribe_openai(path):
    """gpt-4o-transcribe (เร็ว+แม่นกับคำถามสั้น) -> transcript. โยน error -> ตกไป whisper
    ...
    if lao > thai:
        raise RuntimeError("openai STT ถอดเป็นลาว (เสียงไม่ชัด)")
    return text

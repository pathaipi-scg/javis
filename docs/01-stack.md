...
| AI | ดึง JSON เครื่องจักร | Typhoon2 8B (Thai-tuned Llama 3.1, รันบน server ภายในองค์กร, OpenAI-compatible API) |
...
- **LLM**: Typhoon2 8B (`scb10x/llama3.1-typhoon2-8b-instruct`) — รันบน server ภายในองค์กร ข้อมูลไม่ออกนอกองค์กร ฟรี (เรียกผ่าน OpenAI-compatible API: vLLM/Ollama)
  - **ทำไมเปลี่ยนจาก Qwen3-VL มา Typhoon:** (1) Typhoon fine-tune ภาษาไทยมาโดยตรง → เข้าใจ/สรุปไทยและศัพท์ช่างได้ดีกว่า (2) งานนี้เลิกอ่านรูปแล้ว (รูปเป็นแค่ไฟล์แนบ ใช้แค่ caption) จึงไม่ต้องใช้รุ่น VL (3) สลับโมเดลแก้แค่ URL + ชื่อ model ใน `.env` ไม่ต้องแก้โค้ด
  - หมายเหตุ: ตัวแปรใน `.env`/`llm.py` ยังชื่อ `QWEN_*` (ของเดิม) แต่ชี้ไป Typhoon
